...
namespace zm {
	...
	float SDF::Circle(float x, float y, float r) {
		return vec2(x, y).Length() - r;
	}

	float SDF::Square(float x, float y, float r) {
		x = abs(x) - r;
		y = abs(y) - r;
		return vec2(Max(x, 0.f), Max(y, 0.f)).Length() + Min(Max(x, y), 0.f);
	}
	...
	float SDF::Pentagon(float x, float y, float r) {
		...
		p.x = abs(p.x);
		p -= glm::vec2(-k.x, k.y) * Min(glm::dot(glm::vec2(-k.x, k.y), p), 0.f) * 2.f;
		p -= glm::vec2( k.x, k.y) * Min(glm::dot(glm::vec2( k.x, k.y), p), 0.f) * 2.f;
		p -= glm::vec2(Clamp(p.x, -r * k.z, r * k.z), r);
	...
	float SDF::Cube(float x, float y, float z, float r) {
		...
		y = abs(y) - r;
		z = abs(z) - r;
		return Length(vec3(Max(x, 0.f), Max(y, 0.f), Max(z, 0.f))) + Min(Max(x, Max(y, z)), 0.f) - r;
	}
}