...
namespace zm {
	...
	void ValueNoise::Eval(float* x, float* out, uint32_t count) {
		...
		while (count >= 8) {
			simd<float, 8> x_simd(x);
			simd<float, 8> out_simd;
		...
		while (count >= 4) {
			simd<float, 4> x_simd(x);
			simd<float, 4> out_simd;
			Eval(x_simd, out_simd);
			out_simd.unload(out);
...
#if SIMD_FLOAT32_8 == true && SIMD_INT32_8 == true
		while (count >= 8) {
			simd<float, 8> x_simd = simd<float, 8>(simd<int32_t, 8>(x));
			simd<float, 8> out_simd;
			Eval(x_simd, out_simd);
			out_simd.unload(out);
			UpdateStorage(&x, &out, count, 8);
...
#if SIMD_FLOAT32_4 == true && SIMD_INT32_4 == true
		while (count >= 4) {
			simd<float, 4> x_simd = simd<float, 4>(simd<int32_t, 4>(x));
			simd<float, 4> out_simd;
			Eval(x_simd, out_simd);
			out_simd.unload(out);
			UpdateStorage(&x, &out, count, 4);
	...
	void ValueNoise::Eval(int32_t* x, int32_t* y, float* out, uint32_t count) {
		...
		while (count >= 8) {
			simd<float, 8> x_simd = simd<float, 8>(simd<int32_t, 8>(x));
			simd<float, 8> y_simd = simd<float, 8>(simd<int32_t, 8>(y));
			simd<float, 8> out_simd;
			Eval(x_simd, y_simd, out_simd);
			out_simd.unload(out);
			UpdateStorage(&x, &y, &out, count, 8);
		...
		while (count >= 4) {
			simd<float, 4> x_simd = simd<float, 4>(simd<int32_t, 4>(x));
			simd<float, 4> y_simd = simd<float, 4>(simd<int32_t, 4>(y));
			simd<float, 4> out_simd;
			Eval(x_simd, y_simd, out_simd);
			out_simd.unload(out);
			UpdateStorage(&x, &y, &out, count, 4);
	...
	void ValueNoise::Eval(int32_t* x, int32_t* y, int32_t* z, float* out, uint32_t count) {
		...
		while (count >= 8) {
			simd<float, 8> x_simd = simd<float, 8>(simd<int32_t, 8>(x));
			simd<float, 8> y_simd = simd<float, 8>(simd<int32_t, 8>(y));
			simd<float, 8> z_simd = simd<float, 8>(simd<int32_t, 8>(z));
			simd<float, 8> out_simd;
			Eval(x_simd, y_simd, z_simd, out_simd);
			out_simd.unload(out);
			UpdateStorage(&x, &y, &z, &out, count, 8);
		...
		while (count >= 4) {
			simd<float, 4> x_simd = simd<float, 4>(simd<int32_t, 4>(x));
			simd<float, 4> y_simd = simd<float, 4>(simd<int32_t, 4>(y));
			simd<float, 4> z_simd = simd<float, 4>(simd<int32_t, 4>(z));
			simd<float, 4> out_simd;
			Eval(x_simd, y_simd, z_simd, out_simd);
			out_simd.unload(out);
			UpdateStorage(&x, &y, &z, &out, count, 4);
...
#if SIMD_FLOAT32_8 == true && SIMD_INT32_8 == true
	void ValueNoise::Eval(const simd<float, 8>& x, simd<float, 8>& out) {
		x *= m_frequency;
		simd<float, 8> x_floor = floor(x);
		simd<int32_t, 8> x_i(x_floor);
		simd<int32_t, 8> seed(m_seed);
		simd<float, 8> p0, p1;
		WhiteNoise::Eval(x_i + 0, seed, p0);
		WhiteNoise::Eval(x_i + 1, seed, p1);
		simd<float, 8> x_interp = Smoothstep(x - x_floor);
		out = Lerp(p0, p1, x_interp);
	}

	void ValueNoise::Eval(const simd<float, 8>& x, const simd<float, 8>& y, simd<float, 8>& out) {
		x *= m_frequency;
		y *= m_frequency;
		simd<float, 8> x_floor = floor(x);
		simd<float, 8> y_floor = floor(y);
		simd<int32_t, 8> x_i(x_floor);
		simd<int32_t, 8> y_i(y_floor);
		simd<int32_t, 8> seed(m_seed);
		simd<float, 8> p0, p1, p2, p3;
		...
		simd<float, 8> x_interp = Smoothstep(x - x_floor);
		simd<float, 8> y_interp = Smoothstep(y - y_floor);
		simd<float, 8> a = zm::Lerp(p0, p1, x_interp);
		simd<float, 8> b = zm::Lerp(p2, p3, x_interp);
		out = zm::Lerp(a, b, y_interp);
	}

	void ValueNoise::Eval(const simd<float, 8>& x, const simd<float, 8>& y, const simd<float, 8>& z, simd<float, 8>& out) {
		x *= m_frequency;
		y *= m_frequency;
		z *= m_frequency;
		simd<float, 8> x_floor = floor(x);
		simd<float, 8> y_floor = floor(y);
		simd<float, 8> z_floor = floor(z);
		simd<int32_t, 8> x_i(x_floor);
		simd<int32_t, 8> y_i(y_floor);
		simd<int32_t, 8> z_i(z_floor);
		simd<int32_t, 8> seed(m_seed);
		simd<float, 8> p0, p1, p2, p3, p4, p5, p6, p7;
		...
		simd<float, 8> x_interp = Smoothstep(x - x_floor);
		simd<float, 8> y_interp = Smoothstep(y - y_floor);
		simd<float, 8> z_interp = Smoothstep(z - z_floor);
		simd<float, 8> a0 = zm::Lerp(p0, p1, x_interp);
		simd<float, 8> a1 = zm::Lerp(p2, p3, x_interp);
		simd<float, 8> b0 = zm::Lerp(p4, p5, x_interp);
		simd<float, 8> b1 = zm::Lerp(p6, p7, x_interp);
		simd<float, 8> a = zm::Lerp(a0, a1, y_interp);
		simd<float, 8> b = zm::Lerp(b0, b1, y_interp);
...
#if SIMD_FLOAT32_4 == true && SIMD_INT32_4 == true
	void ValueNoise::Eval(simd<float, 4>& x, simd<float, 4>& out) {
		x *= m_frequency;
		simd<float, 4> x_floor = floor(x);
		simd<int32_t, 4> x_i(x_floor);
		simd<int32_t, 4> seed(m_seed);
		simd<float, 4> p0, p1;
		WhiteNoise::Eval(x_i + 0, seed, p0);
		WhiteNoise::Eval(x_i + 1, seed, p1);
		simd<float, 4> x_interp = Smoothstep(x - x_floor);
		out = Lerp(p0, p1, x_interp);
	}

	void ValueNoise::Eval(simd<float, 4>& x, simd<float, 4>& y, simd<float, 4>& out) {
		x *= m_frequency;
		y *= m_frequency;
		simd<float, 4> x_floor = floor(x);
		simd<float, 4> y_floor = floor(y);
		simd<int32_t, 4> x_i(x_floor);
		simd<int32_t, 4> y_i(y_floor);
		simd<int32_t, 4> seed(m_seed);
		simd<float, 4> p0, p1, p2, p3;
		WhiteNoise::Eval(x_i + 0, y_i + 0, seed, p0);
		WhiteNoise::Eval(x_i + 1, y_i + 0, seed, p1);
		WhiteNoise::Eval(x_i + 0, y_i + 1, seed, p2);
		WhiteNoise::Eval(x_i + 1, y_i + 1, seed, p3);
		simd<float, 4> x_interp = Smoothstep(x - x_floor);
		simd<float, 4> y_interp = Smoothstep(y - y_floor);
		simd<float, 4> a = zm::Lerp(p0, p1, x_interp);
		simd<float, 4> b = zm::Lerp(p2, p3, x_interp);
		out = zm::Lerp(a, b, y_interp);
	}

	void ValueNoise::Eval(simd<float, 4>& x, simd<float, 4>& y, simd<float, 4>& z, simd<float, 4>& out) {
		x *= m_frequency;
		y *= m_frequency;
		z *= m_frequency;
		simd<float, 4> x_floor = floor(x);
		simd<float, 4> y_floor = floor(y);
		simd<float, 4> z_floor = floor(z);
		simd<int32_t, 4> x_i(x_floor);
		simd<int32_t, 4> y_i(y_floor);
		simd<int32_t, 4> z_i(z_floor);
		simd<int32_t, 4> seed(m_seed);
		simd<float, 4> p0, p1, p2, p3, p4, p5, p6, p7;
		WhiteNoise::Eval(x_i + 0, y_i + 0, z_i + 0, seed, p0);
		WhiteNoise::Eval(x_i + 1, y_i + 0, z_i + 0, seed, p1);
		WhiteNoise::Eval(x_i + 0, y_i + 1, z_i + 0, seed, p2);
		WhiteNoise::Eval(x_i + 1, y_i + 1, z_i + 0, seed, p3);
		...
		simd<float, 4> x_interp = Smoothstep(x - x_floor);
		simd<float, 4> y_interp = Smoothstep(y - y_floor);
		simd<float, 4> z_interp = Smoothstep(z - z_floor);
		simd<float, 4> a0 = zm::Lerp(p0, p1, x_interp);
		simd<float, 4> a1 = zm::Lerp(p2, p3, x_interp);
		simd<float, 4> b0 = zm::Lerp(p4, p5, x_interp);
		simd<float, 4> b1 = zm::Lerp(p6, p7, x_interp);
		simd<float, 4> a = zm::Lerp(a0, a1, y_interp);
		simd<float, 4> b = zm::Lerp(b0, b1, y_interp);
		out = zm::Lerp(a, b, z_interp);
	}
#endif
}